...
def nslc_eval(indiv, data_pd, k_s, percent_bank, num_neighbours,p):
    ...
    if roll <= percent_bank or indiv_pure_fitness[0] == 0: # Should add something where if this is zero the odds are bumped up by a lot
        # Add to bank
...
def custom_mutate(i1, prob):
    for i in range(0, len(i1)):
        ...
        if rnd < prob:
            i1[i] = (i1[1] + 1) % 2
    if sum(list(i1)) < 2:
        i1[random.randint(0, len(i1) - 1)] = 1
        i1[random.randint(0, len(i1) - 1)] = 1
...
def tda_display(i1, data_pd, true_labels, k_s,umap_data):
    ...
    using_these = []
    feature_names = data_pd.columns.values
    print(len(li1) == len(feature_names))
    for j in range(0, len(feature_names)): # Could be more efficient
        if li1[j] == 1:
            using_these.append(feature_names[j])
    print(using_these)
...
def main():
    data_file = pd.read_csv((r"/home/matthewvandergrift/Desktop/Python_Code/topology/test_data/ionosphere_s.csv"),
                            sep=",")
